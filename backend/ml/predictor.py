...
class Predictor:
    ...
    def __init__(self, model_path='backend/ml/model.pkl', vectorizer_path='backend/ml/vectorizer.pkl'):
        self.model = None
    ...
    def analyze_text(self, text: str) -> Dict:
        ...
        if self.model and self.vectorizer:
            try:
                ...
                return {'original_text': text, 'classification': classification, 'confidence': confidence, 'emotions': emotions, 'rewritten_text': rewritten}
            except Exception as e:
    ...
    def _estimate_emotions(self, text, classification):
        ...
        joy = min(100, joy)
        return {'anger': anger, 'fear': fear, 'sadness': sadness, 'disgust': disgust, 'joy': joy}

    def _generate_rewrite(self, text, classification):
        if classification == 'safe':
            return text or ''
        r = re.sub(r'\\b(stupid|idiot|dumb|moron|trash|suck|worthless|garbage)\\b', 'inappropriate language', text or '', flags=re.IGNORECASE)
        r = re.sub(r'\\b(hate|nazi|racist)\\b', 'discriminatory language', r, flags=re.IGNORECASE)
        r = re.sub(r'\\b(i will kill|i\'ll kill|go die|i hope you die)\\b', 'threatening language', r, flags=re.IGNORECASE)
